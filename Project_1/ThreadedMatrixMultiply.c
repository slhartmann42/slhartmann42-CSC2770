...
int calculationCount = 0; //initialize calculation count at zero
pthread_mutex_t countMutex; //initialize mutex
...
void *multiply(void *arg) {
    ...
    for (int i = 0; i < MATRIX_SIZE; i++) {
        ...
        pthread_mutex_lock(&countMutex); //lock for current thread
        calculationCount ++; //increment count at each multiply
        pthread_mutex_unlock(&countMutex); //unlock for current thread
    }
...
int main() {
    ...
    pthread_mutex_init(&countMutex,NULL); //initialize mutex
